...
def build_memory_guidance(
    ...
    for assessment in applicability_result.memory_assessments:
        if assessment.decision in {"apply", "hint"} and assessment.action_bias:
            if any(float(delta) >= 0 for delta in assessment.action_bias.values()):
    ...
    recommended = [item for item in _dedupe(recommended) if item not in blocked]
    discouraged = [item for item in _dedupe(discouraged) if item not in blocked]
    used_memory_ids = _dedupe(used_memory_ids)
    warning_memory_ids = _dedupe(warning_memory_ids)

    why_not_finalize = ""
    if "FINALIZE_DIAGNOSIS" in blocked:
        why_not_finalize = (
            applicability_result.risk_warning
            or "Diagnosis finalization is blocked by current CaseState or retrieved negative memory."
        )
